...
require.paths.unshift(__dirname + '/../github-flavored-markdown');
var markdown = require('github-flavored-markdown');

/**
 * Library version.
 */
var version = '25.10.11';
...
var render = function(str, file) {
    ...
    for (i = 0, ilen = blocks.length; i < ilen; i++) {
        if (blocks[i].file_overview) {
            file_overview += '<br>' + blocks[i].file_overview;
        }

        //  Support @ignore and @private
        if (blocks[i].ignore || (blocks[i]['private'] && !showPrivate)) {
            continue;
        }
